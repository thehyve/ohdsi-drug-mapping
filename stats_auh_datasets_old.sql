...
SELECT vnr, pname, z_auh.products_v1.drugid, navn, ATCkode
FROM z_auh.products_v1
LEFT JOIN z_auh_druglist ON z_auh_druglist.DRUGID = z_auh.products_v1.DRUGID
ORDER BY RANDOM()
LIMIT 10;
...
FROM z_auh.products_v1
LEFT JOIN z_auh_druglist ON z_auh_druglist.DRUGID = z_auh.products_v1.DRUGID
WHERE atckode IS NULL AND z_auh.products_v1.drugid IS NOT NULL
;
